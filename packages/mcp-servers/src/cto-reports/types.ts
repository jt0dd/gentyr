/**
 * Types for the CTO Reports MCP Server
 *
...
export const ReportToCtoArgsSchema = z.object({
...
export const GetTriageStatsArgsSchema = z.object({
  hours_24h: z.number().optional().default(24).describe('Hours to look back for 24h stats'),
  hours_7d: z.number().optional().default(168).describe('Hours to look back for 7d stats (168 = 7 days)'),
});
...
export type ReportToCtoArgs = z.infer<typeof ReportToCtoArgsSchema>;
...
export const LEGACY_TRIAGE_ACTIONS = [
  ...
  'needs-cto-review',   // Left for CTO to review in /cto session
] as const;
...
export interface ReportToCtoResult {
  id: string;
  message: string;
}
